...
class XYscan:
    ...
    def Xscan(self, X1_counts, X2_counts, X3_counts):
        ...
        for i in range(0,2):
            if self.active_monitor('x', loss0, pos0):
                ...
                while True:
                    d1 = abs(self.hppcontrol.T_get_counts(1, 'x') - x1_final)
                    d2 = abs(self.hppcontrol.T_get_counts(2, 'x') - x2_final)
                    d3 = abs(self.hppcontrol.T_get_counts(3, 'x') - x3_final)
                    if (d1+d2+d3) < self.tolerance * 3:
                        break
                    time.sleep(0.1)
                ...
                if i:
                    ...
                    while True:
                        d1 = abs(self.hppcontrol.T_get_counts(1, 'x') - X1_counts)
                        d2 = abs(self.hppcontrol.T_get_counts(2, 'x') - X2_counts)
                        d3 = abs(self.hppcontrol.T_get_counts(3, 'x') - X3_counts)
                        if (d1+d2+d3) < self.tolerance * 3:
                            break
                        time.sleep(0.1)
    ...
    def Yscan(self, Y1_counts, Y2_counts, Y3_counts):
        ...
        for i in range(0,2):
            if self.active_monitor('y', loss0, pos0):
                ...
                while True:
                    d1 = abs(self.hppcontrol.T_get_counts(1, 'y') - y1_final)
                    d2 = abs(self.hppcontrol.T_get_counts(2, 'y') - y2_final)
                    d3 = abs(self.hppcontrol.T_get_counts(3, 'y') - y3_final)
                    if (d1+d2+d3) < self.tolerance * 3:
                        break
                    time.sleep(0.1)          
                ...
                if i:
                    ...
                    while True:
                        d1 = abs(self.hppcontrol.T_get_counts(1, 'y') - Y1_counts)
                        d2 = abs(self.hppcontrol.T_get_counts(2, 'y') - Y2_counts)
                        d3 = abs(self.hppcontrol.T_get_counts(3, 'y') - Y3_counts)
                        if (d1+d2+d3) < self.tolerance * 3:
                            break
                        time.sleep(0.1)
    ...
    def Xstep(self, x1_o, x2_o, x3_o):
        ...
        while True:
            ...
            while True:
                d1 = abs(self.hppcontrol.T_get_counts(1, 'x') - x1)
                d2 = abs(self.hppcontrol.T_get_counts(2, 'x') - x2)
                d3 = abs(self.hppcontrol.T_get_counts(3, 'x') - x3)
                if (d1+d2+d3) < self.tolerance * 3:
                    break
                time.sleep(0.5)
            if self.final_adjust:
            ...
            if diff <= -bound:
                # go back to the old point, due to backlash, extra 2 counts are added
                x1 = x1 + self.stepScanCounts * self.x_dir + 2 * self.x_dir
                x2 = x2 - self.stepScanCounts * self.x_dir - 2 * self.x_dir
                x3 = x3 - self.stepScanCounts * self.x_dir - 2 * self.x_dir
                trend -= 1
    ...
    def Ystep(self, y1_o, y2_o, y3_o):
        ...
        while True:
            ...
            while True:
                d1 = abs(self.hppcontrol.T_get_counts(1, 'y') - y1)
                d2 = abs(self.hppcontrol.T_get_counts(2, 'y') - y2)
                d3 = abs(self.hppcontrol.T_get_counts(3, 'y') - y3)
                if (d1+d2+d3) < self.tolerance * 3:
                    break
                time.sleep(0.5)
            if self.final_adjust:
            ...
            if diff <= -bound:
                # go back to the old point, due to backlash, extra 2 counts are added
                y1 = y1 + self.stepScanCounts * self.y_dir + 2 * self.y_dir
                y2 = y2 + self.stepScanCounts * self.y_dir + 2 * self.y_dir
                y3 = y3 + self.stepScanCounts * self.y_dir + 2 * self.y_dir
                trend -= 1
                # if trend != 0, exit
                if trend:
                    print('Over')
    ...
    def Xinterp(self, x1_o, x2_o, x3_o):
        ...
        for i in range(1,7):
            ...
            while True:
                d1 = abs(self.hppcontrol.T_get_counts(1, 'x') - x1[i])
                d2 = abs(self.hppcontrol.T_get_counts(2, 'x') - x2[i])
                d3 = abs(self.hppcontrol.T_get_counts(3, 'x') - x3[i])
                if (d1+d2+d3) < self.tolerance * 3:
                    break
                time.sleep(0.5)
    ...
    def Yinterp(self, y1_o, y2_o, y3_o):
        ...
        for i in range(1,7):
            ...
            while True:
                d1 = abs(self.hppcontrol.T_get_counts(1, 'y') - y1[i])
                d2 = abs(self.hppcontrol.T_get_counts(2, 'y') - y2[i])
                d3 = abs(self.hppcontrol.T_get_counts(3, 'y') - y3[i])
                if (d1+d2+d3) < self.tolerance * 3:
                    break
                time.sleep(0.5)
    ...
    def send_to_hpp(self, R):   
        ...
        target_counts = self.hppcontrol.run_to_Tmm(Tmm, Tcounts_old, self.tolerance)
        # print(target_counts)
